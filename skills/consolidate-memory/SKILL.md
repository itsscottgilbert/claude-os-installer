...
description: "Memory consolidation for Claude Code. Scans session transcripts for corrections, decisions, preferences, and patterns, then merges findings into persistent memory files. Auto-triggers via Stop hook every 24hrs. (Renamed from 'dream' so that name is free for Jack Robert's prescriptive dream skill which produces daily action cards.)"
tags: [memory, maintenance, consolidation, autonomous, hook]
---

# Consolidate Memory - Memory Housekeeping for Claude Code

> Memory hygiene that runs while you sleep. Consolidates session learnings into persistent memory.
